...
class GoodsillaSupabaseUser extends BaseAuthUser {
  GoodsillaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> goodsillaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GoodsillaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
